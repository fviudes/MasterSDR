...
_Proposed in [#2502](https://github.com/MasterSDR/MasterSDR/issues/2502) as
a follow-up to [#2456](https://github.com/MasterSDR/MasterSDR/pull/2456).
This document defines the discovery contract; the Qt-side browse
implementation lives in a separate change._

## Scope

MasterSDR speaks TCI to a growing set of LAN peripherals — keyboards
(`mastersdr_pad`), antenna switches (`ShackSwitch`), and over time
...
Rather than host a peripheral-specific UDP responder inside MasterSDR
per device (the pattern `AntennaGeniusModel` follows on port 9007), new
peripherals advertise themselves via mDNS / DNS-SD.  MasterSDR performs
...
MasterSDR's browse code both target.  It is intentionally narrow:
...
- TCI is the protocol MasterSDR speaks to peripherals already
  (`src/core/TciServer.h`, `src/core/TciProtocol.cpp`).  Naming the
  service after the protocol — not the MasterSDR brand — keeps the
...
- Not registered with IANA.  MasterSDR treats this as a de-facto
  convention.  If a future formal registration uses a different name,
  MasterSDR will browse both during a transition period.

### What advertises

The **peripheral** advertises.  MasterSDR is the **browser**.  MasterSDR
...
discovery already published by the radio firmware, and MasterSDR's TCI
...
no fixed port — peripherals pick what suits them.  MasterSDR uses the
...
verbatim in the MasterSDR Peripherals list.  Peripherals SHOULD pick a
...
ignored by MasterSDR; peripherals MAY add vendor-specific keys
(prefixed `x-`) for their own purposes.

| Key           | Required | Value                                                                                               | Example                  |
| ------------- | -------- | --------------------------------------------------------------------------------------------------- | ------------------------ |
| `model`       | yes      | Stable machine-readable model identifier.  Lowercase, hyphen-separated.  Not a marketing name.      | `shackswitch-v2`, `mastersdr-pad` |
...
| `x-*`         | no       | Vendor-specific keys.  MasterSDR ignores these but may surface them in a "details" view in future. | `x-serial=AB-12-34`      |

### `class` values

The class is what MasterSDR uses to route a discovered peripheral to
the right settings sub-panel and applet.  The set is closed — adding a
new class is a docs PR, not a peripheral-side decision.

| Value             | Meaning                                                            |
| ----------------- | ------------------------------------------------------------------ |
| `controller`      | Keyboard / panel / generic operator interface (e.g. `mastersdr_pad`). |
| `antenna-switch`  | Antenna selector / matrix (e.g. `ShackSwitch`).                    |
| `rotator`         | Antenna rotator.                                                   |
| `amplifier`       | RF amplifier.                                                      |
| `tuner`           | Antenna tuner.                                                     |
| `other`           | Anything that doesn't fit the above; MasterSDR shows it under "Other peripherals". |

If `class` is missing or unrecognised, MasterSDR treats it as `other`.
...
- A peripheral missing `model`, `class`, `tci-version`, or `txtvers`
  ...
  so operators can see that *something* is responding, but MasterSDR
  will not auto-route it to a class-specific applet.
- MasterSDR MUST NOT crash, refuse to populate the list, or hide other
...
MasterSDR's browse will pick this up, show `Shack rotor wall` in the
...
MasterSDR currently discovers `Antenna Genius` via a UDP listener on
port 9007 (`src/models/AntennaGeniusModel.cpp`).  That path is **not
removed** by this proposal:

- The AG firmware broadcasts on 9007; MasterSDR continues to listen
  for it.  Changing the AG side is out of MasterSDR's control.
- The mDNS path is **additive** for peripherals that have not shipped
  yet (`mastersdr_pad`) or that choose to advertise both ways
  (`ShackSwitch`).
- A peripheral that advertises via mDNS AND replies to AG-style
  broadcasts will appear once in the list — MasterSDR deduplicates by
...
- A future `txtvers=2` would mean key semantics have changed
  incompatibly.  MasterSDR will continue to parse `txtvers=1`
...
- Authentication / authorisation between MasterSDR and the peripheral.
