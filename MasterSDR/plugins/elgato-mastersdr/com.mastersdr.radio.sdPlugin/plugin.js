/**
 * mastersdr Stream Deck Plugin
 * Controls FlexRadio via mastersdr's TCI WebSocket server.
...
const TUNE_STEP_HZ = 1000; // 1 kHz per press; matches streamcontroller-mastersdr
...
const actionHandlers = {
    // TX
    "com.mastersdr.radio.ptt":         { keyDown: () => tciSend("trx:0,true;"),  keyUp: () => tciSend("trx:0,false;") },
    "com.mastersdr.radio.mox-toggle":  { keyDown: () => tciSend(`trx:0,${!radio.transmitting};`) },
    "com.mastersdr.radio.tune-toggle": { keyDown: () => tciSend(`tune:0,${!radio.tuning};`) },
    "com.mastersdr.radio.rf-power":    { keyDown: () => tciSend(`drive:${nextInCycle(POWER_LEVELS, radio.rfPower)};`) },
    "com.mastersdr.radio.tune-power":  { keyDown: () => tciSend(`tune_drive:${nextInCycle(TUNE_LEVELS, radio.tunePower)};`) },
    // Audio
    "com.mastersdr.radio.mute-toggle":  { keyDown: () => tciSend(`mute:0,${!radio.muted};`) },
    "com.mastersdr.radio.volume-up":    { keyDown: () => tciSend(`volume:${Math.min(radio.volume + 5, 100)};`) },
    "com.mastersdr.radio.volume-down":  { keyDown: () => tciSend(`volume:${Math.max(radio.volume - 5, 0)};`) },
    // DSP
    "com.mastersdr.radio.nb-toggle":  { keyDown: () => tciSend(`rx_nb:0,${!radio.nbOn};`) },
    "com.mastersdr.radio.nr-toggle":  { keyDown: () => tciSend(`rx_nr:0,${!radio.nrOn};`) },
    "com.mastersdr.radio.anf-toggle": { keyDown: () => tciSend(`rx_anf:0,${!radio.anfOn};`) },
    "com.mastersdr.radio.apf-toggle": { keyDown: () => tciSend(`rx_apf:0,${!radio.apfOn};`) },
    "com.mastersdr.radio.sql-toggle": { keyDown: () => tciSend(`sql_enable:0,${!radio.sqlOn};`) },
    // Slice
    "com.mastersdr.radio.split-toggle": { keyDown: () => tciSend(`split_enable:0,${!radio.split};`) },
    "com.mastersdr.radio.lock-toggle":  { keyDown: () => tciSend(`lock:0,${!radio.locked};`) },
    "com.mastersdr.radio.rit-toggle":   { keyDown: () => tciSend(`rit_enable:0,${!radio.ritOn};`) },
    "com.mastersdr.radio.xit-toggle":   { keyDown: () => tciSend(`xit_enable:0,${!radio.xitOn};`) },
    // Frequency
    "com.mastersdr.radio.tune-up":   { keyDown: () => tciSend(`vfo:0,0,${radio.frequency + TUNE_STEP_HZ};`) },
    "com.mastersdr.radio.tune-down": { keyDown: () => tciSend(`vfo:0,0,${radio.frequency - TUNE_STEP_HZ};`) },
    "com.mastersdr.radio.band-up":   { keyDown: () => { const i = Math.min(closestBandIndex(radio.frequency) + 1, BAND_ORDER.length - 1); tciSend(`vfo:0,0,${BANDS[BAND_ORDER[i]]};`); } },
    "com.mastersdr.radio.band-down": { keyDown: () => { const i = Math.max(closestBandIndex(radio.frequency) - 1, 0); tciSend(`vfo:0,0,${BANDS[BAND_ORDER[i]]};`); } },
    // DVK
    "com.mastersdr.radio.dvk-play":   { keyDown: () => tciSend("rx_play:0,true;") },
    "com.mastersdr.radio.dvk-record": { keyDown: () => tciSend("rx_record:0,true;") },
};

// Band actions
for (const [band, freq] of Object.entries(BANDS)) {
    actionHandlers[`com.mastersdr.radio.band-${band}`] = { keyDown: () => tciSend(`vfo:0,0,${freq};`) };
}

// Mode actions
for (const mode of ["USB", "LSB", "CW", "AM", "FM", "DIGU", "DIGL", "FT8"]) {
    actionHandlers[`com.mastersdr.radio.mode-${mode.toLowerCase()}`] = { keyDown: () => tciSend(`modulation:0,${mode};`) };
}
...
console.log("mastersdr Stream Deck plugin started");
