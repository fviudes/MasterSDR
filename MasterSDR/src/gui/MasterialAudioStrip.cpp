...
MasterialAudioStrip::MasterialAudioStrip(AudioEngine* engine, QWidget* parent)
    ...
    , m_presets(new ChannelStripPresets(engine, this))
{
    ...
    const QString title = QString::fromUtf8("Masterial Audio \xe2\x80\x94 Channel Strip");
    ...
    // wires them to the same PUDU monitor as the docked ClientChainApplet.
    {
        ...
        connect(m_rxBtn, &QPushButton::toggled, this, [this](bool on) {
            ...
            if (m_titleLbl) {
                const QString base =
                    QString::fromUtf8("Masterial Audio \xe2\x80\x94 Channel Strip");
            ...
            AppSettings::instance().setValue("MasterialStripMode",
                                             on ? "RX" : "TX");
        });
        ...
        connect(m_chain, &StripChainWidget::stageEnabledChanged,
                this,    &MasterialAudioStrip::stageEnabledChanged);
        ...
        connect(m_chainRx, &StripRxChainWidget::stageEnabledChanged,
                this,      &MasterialAudioStrip::rxStageEnabledChanged);
        connect(m_chainRx, &StripRxChainWidget::dspEditRequested,
                this,      &MasterialAudioStrip::rxDspEditRequested);
        connect(m_chainRx, &StripRxChainWidget::editRequested,
                this,      &MasterialAudioStrip::rxStageEditRequested);
        ...
        connect(m_bypassBtn, &QPushButton::toggled,
                this, &MasterialAudioStrip::onBypassToggled);
        ...
        connect(m_monRecBtn, &QPushButton::clicked,
                this, &MasterialAudioStrip::monitorRecordClicked);
        ...
        connect(m_monPlayBtn, &QPushButton::clicked,
                this, &MasterialAudioStrip::monitorPlayClicked);
        ...
        connect(m_presetCombo,
                ...
                this, &MasterialAudioStrip::onPresetComboActivated);
        ...
        connect(m_presetSaveBtn, &QPushButton::clicked,
                this, &MasterialAudioStrip::doSavePreset);
        ...
        connect(m_presetDeleteBtn, &QPushButton::clicked,
                this, &MasterialAudioStrip::doDeletePreset);
        ...
        const QString lastActive = AppSettings::instance()
            .value("MasterialStripActivePreset", "").toString();
    ...
    // ADSP embeds MasterDspWidget directly (already a reusable widget,
    // shared with MasterDspDialog and the docked applet).  DESS has no
    ...
    m_adspRx = new MasterDspWidget(m_audio, this);
    // Match Settings → MasterDsp Settings... — 13 px fonts, full-size
    ...
    connect(m_eq, &StripEqPanel::cutoffsDragRequested,
            this, &MasterialAudioStrip::cutoffsDragRequested);
    ...
    // BYPASS target, and title-bar suffix.
    {
        const QString m = AppSettings::instance()
            .value("MasterialStripMode", "TX").toString();
        if (m == "RX" && m_rxBtn) {
...
MasterialAudioStrip::~MasterialAudioStrip() = default;

void MasterialAudioStrip::setFramelessMode(bool on)
{
...
void MasterialAudioStrip::setTxFilterCutoffs(int lowHz, int highHz)
{
...
void MasterialAudioStrip::setMonitorRecording(bool on)
{
    m_monRecording = on;
    if (m_monRecBtn) {
...
void MasterialAudioStrip::setMonitorPlaying(bool on)
{
    m_monPlaying = on;
    if (m_monPlayBtn) {
...
void MasterialAudioStrip::setMonitorHasRecording(bool has)
{
...
void MasterialAudioStrip::setMicInputReady(bool ready)
{
    if (m_chain) m_chain->setMicInputReady(ready);
}

void MasterialAudioStrip::setTxActive(bool active)
{
    if (m_chain) m_chain->setTxActive(active);
}

void MasterialAudioStrip::setRxPcAudioEnabled(bool on)
{
    if (m_chainRx) m_chainRx->setPcAudioEnabled(on);
}

void MasterialAudioStrip::setRxClientDspActive(bool on, const QString& label)
{
    if (m_chainRx) m_chainRx->setClientDspActive(on, label);
}

void MasterialAudioStrip::setRxOutputUnmuted(bool on)
{
    if (m_chainRx) m_chainRx->setOutputUnmuted(on);
}

void MasterialAudioStrip::refreshChainPaint()
{
    if (m_chain) m_chain->update();
    if (m_chainRx) m_chainRx->update();
}

void MasterialAudioStrip::saveGeometryToSettings()
{
    if (m_restoring) return;
    auto& s = AppSettings::instance();
    s.setValue("MasterialStripGeometry", saveGeometry().toBase64());
    s.save();
}

void MasterialAudioStrip::restoreGeometryFromSettings()
{
    auto& s = AppSettings::instance();
    const QByteArray geom = QByteArray::fromBase64(
        s.value("MasterialStripGeometry").toString().toUtf8());
...
void MasterialAudioStrip::closeEvent(QCloseEvent* ev)
{
    saveGeometryToSettings();
    auto& s = AppSettings::instance();
    s.setValue("MasterialStripVisible", "False");
    s.save();
    QWidget::closeEvent(ev);
}

void MasterialAudioStrip::moveEvent(QMoveEvent* ev)
{
    saveGeometryToSettings();
    QWidget::moveEvent(ev);
}

void MasterialAudioStrip::resizeEvent(QResizeEvent* ev)
{
    saveGeometryToSettings();
    QWidget::resizeEvent(ev);
}

void MasterialAudioStrip::showEvent(QShowEvent* ev)
{
    auto& s = AppSettings::instance();
    s.setValue("MasterialStripVisible", "True");
    s.save();
    QWidget::showEvent(ev);
}

void MasterialAudioStrip::hideEvent(QHideEvent* ev)
{
    auto& s = AppSettings::instance();
    s.setValue("MasterialStripVisible", "False");
    s.save();
...
Qt::Edges MasterialAudioStrip::edgesAt(const QPoint& pos) const
{
...
void MasterialAudioStrip::updateResizeCursor(const QPoint& pos)
{
...
void MasterialAudioStrip::mouseMoveEvent(QMouseEvent* ev)
{
...
void MasterialAudioStrip::mousePressEvent(QMouseEvent* ev)
{
...
void MasterialAudioStrip::leaveEvent(QEvent* ev)
{
    setCursor(Qt::ArrowCursor);
    QWidget::leaveEvent(ev);
}

bool MasterialAudioStrip::eventFilter(QObject* obj, QEvent* ev)
{
...
void MasterialAudioStrip::onBypassToggled(bool checked)
{
    if (!m_audio) return;
...
void MasterialAudioStrip::rebuildPresetCombo(const QString& selectName)
{
...
void MasterialAudioStrip::updatePresetButtonEnable()
{
...
void MasterialAudioStrip::onPresetComboActivated(int idx)
{
    ...
    if (m_presets->loadPreset(name)) {
        ...
        AppSettings::instance().setValue("MasterialStripActivePreset", name);
        refreshAllPanelsFromEngine();
    }
    updatePresetButtonEnable();
}

void MasterialAudioStrip::refreshAllPanelsFromEngine()
{
...
void MasterialAudioStrip::doImportPreset()
{
    ...
    const QString path = QFileDialog::getOpenFileName(
        ...
        defaultDir,
        tr("Channel strip preset (*.json *.masterpreset);;All files (*)"));
    ...
    if (m_presets->loadPreset(imported)) {
        ...
        AppSettings::instance().setValue("MasterialStripActivePreset", imported);
        refreshAllPanelsFromEngine();
    }
    rebuildPresetCombo(m_currentPresetName);
}

void MasterialAudioStrip::doExportPreset()
{
...
void MasterialAudioStrip::doExportLibrary()
{
    if (!m_presets) return;
    ...
    const QString defaultDir = QStandardPaths::writableLocation(
        QStandardPaths::DocumentsLocation);
    const QString suggested = defaultDir + "/MasterSDR-ChannelStrip-Library.json";
    const QString path = QFileDialog::getSaveFileName(
...
void MasterialAudioStrip::doSavePreset()
{
    if (!m_presets) return;
...
void MasterialAudioStrip::doDeletePreset()
{
    ...
    m_currentPresetName.clear();
    AppSettings::instance().remove("MasterialStripActivePreset");
    rebuildPresetCombo();
}
